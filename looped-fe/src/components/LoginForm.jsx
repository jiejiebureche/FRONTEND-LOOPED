...
function LoginForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await api.post("/users/api/login/", {
        email,
        password,
      });
    ...
    <>
      <div className="flex h-screen w-screen bg-[url(./assets/looped-bg.png)] bg-cover bg-center bg-no-repeat items-center justify-center">
        <form
          ...
          className="login-form flex flex-col bg-white w-3/4 h-3/5 sm:w-3/5 sm:h-2/3 md:w-2/3 md:h-3/5 lg:w-1/3 lg:h-2/3 text-[2.2vh] rounded-[10px] items-center justify-center-safe"
        >
